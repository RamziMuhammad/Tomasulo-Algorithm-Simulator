...
function Address(register, offset) {
  let addr = {
    ...
    toString: function () {
      if (defined(offset)) {
        let sign = offset < 0 ? " - " : " + ";
        return "Regs[" + register.toString() + "]" + sign + offset.toString();
      }
...
function RegisterStat(registers) {
  ...
  let table = newTable({
    ...
    id: "RegisterStatus",
    headings: ["Field"].concat(
      _.map(registers, function (r) {
        return r.name;
      })
    ),
  });
  ...
  function makeStatusSlot(reg) {
    ...
    slot.highlightSource = function () {
      if (slot.full()) $(cell).addClass("sourceOccupied");
      else $(cell).addClass("source");
    };
    slot.highlightDestination = function () {
      if (slot.full()) $(cell).toggleClass("destinationOccupied");
      else $(cell).toggleClass("destination");
    };
  ...
  rs.each = function* () {
    for (let r of registerSlots) yield r;
  };
  return rs;
}
...
function ExecutionUnit(tr, type, name, registerStatus) {
  function makeProperty(name, initialValue, hover) {
    ...
    return function (v) {
      if (defined(v)) {
        $(cell).text((value = v) === null ? "" : v.toString());
      }
      return value;
    };
  }
  ...
  object.waitingOn = function (executionUnit) {
    ...
    if (null === object.Qj() || null === object.Qk()) return false;
...
let ReservationStation = function (registerStatus) {
  ...
  object.each = function* () {
    for (let unit of executionUnits) yield unit;
  };
  ...
  function IINST(op, type, rd, rs, offset) {
    ...
    return {
      ...
      toString: function () {
        ...
        else return op + " " + rs + "," + offset.toString() + "[" + rd + "]";
      },
      destination: function () {
        return registerStatus[rd];
      },
...
let InstructionStatusTable = function (reservationStation) {
  ...
  let object = {
    read: function (instruction) {
      new InstructionStatus(reservationStation, table, instruction);
    },
  };
  return object;
};
